...
network make_network(int n)
{
    ...
    net.gt_seen = calloc(1, sizeof(int));
    #ifdef GPU
...
{
#ifdef GPU
    if(gpu_index >= 0) return train_network_joint_datum_gpu(net, contNet, joinNet, x, y, x_f, y_f);
#endif
    network_state state;
    *net.seen += net.batch;
    state.index = 0;
...
float train_joint_network(network net, network contNet, network join, data d, data d_f)
{
    ...
    float *X = calloc(batch*d.X.cols, sizeof(float));
    float *y = calloc(batch*d.y.cols, sizeof(float));
    float *X_f = calloc(batch*d_f.X.cols, sizeof(float));
    float *y_f = calloc(batch*d_f.y.cols, sizeof(float));

    int i;
    float sum = 0;
    for(i = 0; i < n; ++i){
        get_next_batch(d, batch, i*batch, X, y);
        get_next_batch(d_f, batch, i*batch, X_f, y_f);
        float err = train_joint_network_datum(net, contNet, join, X, y, X_f, y_f);
        sum += err;
    }
    free(X_f);
    free(y_f);
