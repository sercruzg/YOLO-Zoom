...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[concat_bottom]")==0) return CONCAT_BOTTOM;
    return BLANK;
}
...
concat_bottom_layer parse_concat_bottom(list *options, size_params params)
{
    int batch,h,w,c,l, flip;
    h = option_find_int(options, "height",1);
    w = option_find_int(options, "width",1);
    c = option_find_int(options, "channels",1);
    
    int h_bot = option_find_int(options, "height_bot",1);
    int w_bot = option_find_int(options, "width_bot",1);
    int c_bot = option_find_int(options, "channels_bot",1);
    
    int w_step = option_find_int(options, "w_step",1);
    int h_step = option_find_int(options, "h_step",1);
    
    l = option_find_int(options, "layer",0);
    flip = option_find_int(options, "flip",0);
    batch=params.batch;

    concat_bottom_layer layer = make_concat_bottom_layer(batch, h ,w, c, l, flip, h_bot, w_bot, c_bot, h_step, w_step);
...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    layer.mask = option_find_int_quiet(options, "mask", 0);
    if(params.net.adam){
...
layer parse_region(list *options, size_params params)
{
    ...
    layer l;
    if(spatial == 0){
        l = make_region_layer(params.batch, params.w, params.h, num, classes, coords, max_boxes);
    }
    else
        l = make_region_layer(params.batch, spatial, spatial, num, classes, coords, max_boxes);
    assert(l.outputs == params.inputs);

    l.log = option_find_int_quiet(options, "log", 0);
    l.sqrt = option_find_int_quiet(options, "sqrt", 0);
    l.context = option_find_int_quiet(options, "context", 0);
    ...
    l.pointLayer = option_find_int(options, "layer",-1);
...
void parse_net_options(list *options, network *net)
{
    ...
    net->div = option_find_int_quiet(options, "div",1);
    ...
    net->flip = option_find_int_quiet(options, "flip", 0);
    ...
    int gtMax = option_find_int(options, "gtMax", 0);
    int imgs = net->batch * net->subdivisions;
    net->gtMax = gtMax;
    fprintf(stderr, "imgs: %d, classes: %d\n", imgs,gtMax);
    net->gt = calloc(imgs*gtMax*4, sizeof(float));
}
...
network parse_network_cfg_custom(char *filename, int batch)
{
    ...
    while(n){
        ...
        if(lt == CONCAT_BOTTOM){
            l = parse_concat_bottom(options, params);
...
void save_deformation_weights(layer l, FILE *fp)
{
    if(l.binary){
...
#endif
    int num = l.out_c - (l.num_parts * l.c);
    fwrite(l.mean, sizeof(float), l.n, fp);
    fwrite(l.variance, sizeof(float), num, fp);
}
...
void save_weights_upto(network net, char *filename, int cutoff)
{
    ...
    fwrite(net.gt_seen, sizeof(int), 1, fp);
    fclose(fp);
}
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    if ((major * 10 + minor) >= 2) {
		...
		*net->gt_seen = iseen;
		...
		printf("\n seen gt 32 \n");
		fread(net->gt_seen, sizeof(int), 1, fp);
	}*/
    fprintf(stderr, "Done!\n");
    fclose(fp);
}
