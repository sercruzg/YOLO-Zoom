...
__global__ void im2max_gpu_kernel(const int n, float* in_im, int* indexes, float* flat,
    ...
    int flat_offset = width*height*channels*ksize*ksize;
    ...
    for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                int cur_h = h + i + offset;
                int cur_w = w + j + offset;

                int index_im = cur_w + width*(cur_h + height*(k + b*channels));
                int index_flat = ksize*ksize*(h*width*channels + w*channels + k) + b*flat_offset + flat_i;

                int valid = (cur_h >= 0 && cur_h < height &&
                    cur_w >= 0 && cur_w < width);
                float val = (valid != 0) ? flat[index_flat] : -INFINITY;
                max_i = (val > max) ? index_im : max_i;
                max_value = (val > max) ? in_im[index_im] : max_value;
                max   = (val > max) ? val   : max;
                flat_i++;
        }
    }
    int out_index = w + width*(h + height*(k + channels*b));
    out_im[out_index] = max_value;
    indexes[out_index] = max_i;
}

void im2max_ongpu(float *in_im, int * indexes, float * flat,
        int height, int width, int channels, int batch,
        int ksize, int pad, float *out_im){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    size_t num_kernels = channels * height * width * batch;
    im2max_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, in_im, indexes, flat, height, width, channels, ksize, pad, out_im);
}


/*
__global__ void im2flat_gpu_kernel(const int n, float* in_im,
        const int height, const int width, const int channels, const int ksize, const int pad,
        float *out_flat) {
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= n) return;
    ...
    int outputs = width*height*channels*ksize*ksize;
    int offset = -pad;
    int att_i = 0;
    ...
    for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                ...
                int index_im = cur_w + width*(cur_h + height*(k + b*channels));
                int index_flat = ksize*ksize*(h*width*channels + w*channels + k) + b*outputs + att_i;

                int valid = (cur_h >= 0 && cur_h < height &&
                    cur_w >= 0 && cur_w < width);
                float value = (valid != 0) ? in_im[index_im] : 0;
                out_flat[index_flat] = value;
                att_i++;
        }
    }
}

void im2flat_ongpu(float *in_im,
        int height, int width, int channels, int batch,
        int ksize, int pad, float *out_im){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    size_t num_kernels = channels * height * width * batch;
    im2flat_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, in_im, height, width, channels, ksize, pad, out_im);
}*/

__global__ void flat2im_gpu_kernel(const int n, float* in_flat,
        const int height, const int width, const int channels, const int ksize, const int pad,
        float *out_im) {
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= n) return;
    ...
    int index_flat = ksize*ksize*(h*width*channels + w*channels + k) + b*outputs;

    for(int i = 0; i < ksize * ksize; i++){
        d += in_flat[index_flat + i];
    }
    int index_im = w + width*(h + height*(k + b*channels));
    out_im[index_im] += d;
}

void flat2im_ongpu(float *in_im,
        int height, int width, int channels, int batch,
        int ksize, int pad, float *out_im){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    size_t num_kernels = channels * height * width * batch;
    flat2im_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, in_im, height, width, channels, ksize, pad, out_im);
}

__global__ void im2max_back_gpu_kernel(const int n, float* in_im, int* indexes,
        const int height, const int width, const int channels, const int ksize, const int pad,
        float *out_im) {
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= n) return;
    ...
    for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                ...
                int valid = (cur_w >= 0 && cur_w < w &&
                     cur_h >= 0 && cur_h < h);
                d += (valid && indexes[index_im] == id) ? in_im[index_im] : 0;
        }
    }
    out_im[id] += d;
}

void im2max_back_ongpu(float *in_im, int * indexes,
        int height, int width, int channels, int batch,
        int ksize, int pad, float *out_im){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    size_t num_kernels = channels * height * width * batch;
    im2max_back_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, in_im, indexes, height, width, channels, ksize, pad, out_im);
}

__global__ void im2elemcol_gpu_kernel(const int n, float* data_im,
    ...
    int att = 0;
    int c_out = c * ksize * ksize;
    for (int i = 0; i < ksize; ++i) {
            for (int j = 0; j < ksize; ++j) {
                int c_col = c_out + att;
                int h_im = h + i + offset;
                int w_im = w + j + offset;
                int im_index = (c * height + h_im) * width + w_im;
                int col_index = (c_col * height + h) * width + w;

                data_col_ptr[col_index] = (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ? data_im_ptr[im_index] : 0;
                att++;
        }
    }
...
void im2elemcol_ongpu(float *data_im,
        int channels, int height, int width,
        int ksize, int pad, float *data_col){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    int num_kernels = channels * height * width;
    im2elemcol_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, data_im, height, width, ksize, pad, data_col);
}

__global__ void elemcol2im_gpu_kernel(const int n, float* data_col,
    ...
    int im_index = (c * height + h) * width + w;
    data_im_ptr[im_index] = 0;

    for(int i = 0; i < ksize * ksize; i++){
        int c_out = c * ksize * ksize + i;
        int col_index = (c_out * height + h) * width + w;
        data_im_ptr[im_index] += data_col_ptr[col_index];
    }
}

void elemcol2im_ongpu(float *data_col,
        int channels, int height, int width,
        int ksize, float *data_im){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    int num_kernels = channels * height * width;
    elemcol2im_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, data_col, height, width, ksize, data_im);
}

__global__ void flatten2col_gpu_kernel(const int n, float* data_flat,
        const int height, const int width, const int channels, const int classes,
        float *data_col, const int forward) {
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= n) return;
...
int index = size*(j*l.w*l.n + i*l.n + n) + b*l.outputs;*/
    ...
    for(int i = 0; i < classes; i++){
        ...
        int c_col = c * classes + i;
        int col_index = (c_col * height + h) * width + w;
        int flatIndex = classes*(h*width*channels + w*channels + c) + i;

        if(forward)
            data_col_ptr[col_index] = data_flat_ptr[flatIndex];
        else
            data_flat_ptr[flatIndex] = data_col_ptr[col_index];
...
void flatten2col_ongpu(float *im,
         int channels, int height, int width,
         int classes, float *data_col, int forward){
    // We are going to launch channels * height_col * width_col kernels, each
    // kernel responsible for copying a single-channel grid.
    int num_kernels = channels * height * width;
    //fprintf(stderr, "h=%d w=%d channels=%d classes=%d\n", height, width, channels, classes);
    flatten2col_gpu_kernel<<<cuda_gridsize(num_kernels), BLOCK>>>(
                num_kernels, im, height, width, channels, classes,
                data_col, forward);
}
...
__device__ void hardmax_device(int n, float *input, float temp, float *output)
{
    int i;
    float largest = -INFINITY;
    int i_largest = 0;
    for(i = 0; i < n; ++i){
        int val = input[i];
        i_largest = (val>largest) ? i : i_largest;
        largest = (val>largest) ? val : largest;
    }
    for(i = 0; i < n; ++i){
        output[i] = (i == i_largest) ? 1 : 0;
    }
}
...
__global__ void hardmax_kernel(int n, int offset, int batch, float *input, float temp, float *output)
{
    int b = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(b >= batch) return;
    softmax_device(n, input + b*offset, temp, output + b*offset);
...
extern "C" void hardmax_gpu(float *input, int n, int offset, int groups, float temp, float *output)
{
    int inputs = n;
    int batch = groups;
    softmax_kernel<<<cuda_gridsize(batch), BLOCK, 0, get_cuda_stream()>>>(inputs, offset, batch, input, temp, output);
