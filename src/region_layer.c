...
void get_final_output(const region_layer l, float *output){
    int i,j,b,t,n;
    ...
    for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    output[index + 0] = (i + logistic_activate(l.output[index + 0])) / l.w;
                    output[index + 1] = (j + logistic_activate(l.output[index + 1])) / l.h;
                    output[index + 2] = exp(l.output[index + 2]) * l.biases[2*n];
                    output[index + 3] = exp(l.output[index + 3]) * l.biases[2*n+1];
                }
            }
    }
}
...
//         if(l.context){
//             int outRegion[2][l.w][l.h];
//             for(int gtCount = 0; gtCount < 2; gtCount++){
//                 for(int fillW = 0; fillW < l.w; fillW++){
//                     for(int fillH = 0; fillH < l.h; fillH++){
//                         outRegion[gtCount][fillW][fillH] = 0;
//                     }
//                 }
//             }
//             int bW,bH;
...
//                 bW = (truth.w * l.w);
//                 bH = (truth.h * l.h);
//                 for(int bcW = i-bW/2; bcW <= i+bW/2; bcW++){
//                     for(int bcH = j-bH/2; bcH <= j+bH/2; bcH++){
//                         int outSelW = fmaxf(bcW -1,0);
//                         int outSelH = fminf(bcH +1,l.h-1);
//                         outRegion[t][outSelW][outSelH] = 1;
//                         
//                         outSelW = fmaxf(bcW ,0);
//                         outSelH = fminf(bcH +1,l.h-1);
//                         outRegion[t][outSelW][outSelH] = 1;
//                         
//                         outSelW = fminf(bcW +1,l.w-1);
//                         outSelH = fminf(bcH +1,l.h-1);
//                         outRegion[t][outSelW][outSelH] = 1;
//                         
//                         outSelW = fminf(bcW +1,l.w-1);
...
//                 for(int bcW = i-bW/2; bcW <= i+bW/2; bcW++){
//                     for(int bcH = j-bH/2; bcH <= j+bH/2; bcH++){
//                         outRegion[0][bcW][bcH] = 0;
//                         outRegion[1][bcW][bcH] = 0;
//                     }
//                 }
//             }
//             //
...
//                             truth_shift.x = 0;
//                             truth_shift.y = 0;
//                             //printf("index %d %d lH %d lW %d    truth %d\n",i, j, l.h, l.w, t);
//                             for(n = 0; n < l.n; ++n){
//                                 int index = size*(j*l.w*l.n + i*l.n + n) + b*l.outputs;
//                                 box pred = get_region_boxImage(l.output, l.biases, n, index, l.w, l.h);
//                                 if(l.bias_match){
//                                     pred.w = l.biases[2*n];
...
//                                 }
//                             }
//                             float iou = delta_region_boxImage(truth, l.output, l.biases, best_n, best_index, l.w, l.h, l.delta, l.coord_scale);
//                             if(iou > .5) found = 1;
...
//                 ++count;
//                 avg_obj += l.output[gtBest_index + 4];
// //                 l.delta[best_index + 4] = l.object_scale * (1 - l.output[best_index + 4]) * logistic_gradient(l.output[best_index + 4]);
// //                 if (l.rescore) {
// //                     l.delta[best_index + 4] = l.object_scale * (found_iou - l.output[best_index + 4]) * logistic_gradient(l.output[best_index + 4]);
// //                 }
// 
// 
// //                 int class_id = state.truth[t*5 + b*l.truths + 4];
...
void forward_region_layer_gpu(const region_layer l, network_state state)
{
    ...
    //get_final_output(l, in_cpu);
    //cuda_push_array(l.output_gpu, in_cpu, l.batch*l.outputs);
