...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show)
{
    ...
    while(get_current_batch(net) < net.max_batches){
        ...
        for (int count = 0; count < imgs*net.gtMax*4; ++count){
            net.gt[count] = -1;
        }
        
        int gtCount = 0;
        for (int batchNum = 0; batchNum < args.n; ++batchNum){
           int k;
           for(k = 0; k < l.max_boxes; ++k){
           box b = float_to_box(train.y.vals[batchNum] + k*5);
           if(!b.x) break;
           gtCount = net.gtMax*4*batchNum + k*4;
           net.gt[gtCount] = b.x;
           net.gt[gtCount + 1] = b.y;
           net.gt[gtCount + 2] = b.w;
           net.gt[gtCount + 3] = b.h;
//            printf("loaded: %f %f %f %f Size: %d\n", net.gt[gtCount], net.gt[gtCount + 1], net.gt[gtCount + 2], net.gt[gtCount + 3], args.n);
//            sprintf(mess, "truth%d", batchNum);
//            image im = float_to_image(416, 416, 3, train.X.vals[batchNum]);
...
void fill_truthGrad_detection(char *path, int num_boxes, float *truth)
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
...
void train_joint_bottom(char *datacfg, char *cfgfile, char *weightfile,  char *weightSecondfile, char *bottomArchNet, int *gpus, int ngpus, int clear, int dont_show, char *jointArchNet)
{
    list *options = read_data_cfg(datacfg);
    char *train_images = option_find_str(options, "train", "data/train.list");
    char *backup_directory = option_find_str(options, "backup", "/backup/");
    ...
    srand(time(0));
    char *base = basecfg(jointArchNet);
    printf("%s\n", base);
    float avg_loss = -1;
    network *nets = calloc(ngpus, sizeof(network));
    network *bottomNets = calloc(ngpus, sizeof(network));
    ...
    for(i = 0; i < ngpus; ++i){
...
#endif
        nets[i] = parse_network_cfg(cfgfile);
        bottomNets[i] = parse_network_cfg(bottomArchNet);
        joinNets[i] = parse_network_cfg(jointArchNet);
        if(weightfile){
            load_weights(&nets[i], weightfile);
            load_weights(&bottomNets[i], weightSecondfile);
        }
        if(clear){ 
            *nets[i].seen = 0;
            *bottomNets[i].seen = 0;
        ...
        nets[i].learning_rate *= ngpus;
        bottomNets[i].learning_rate *= ngpus;
        joinNets[i].learning_rate *= ngpus;
    }
    srand(time(0));
    network net = nets[0];
    network bottomNet = bottomNets[0];
    ...
    data train, buffer;
    data train_bottom, buffer_bottom;
    ...
    args.w_bot = bottomNet.w;
    args.h_bot = bottomNet.h;
    args.div = joinNet.div;
	...
	args.small_object = l.small_object;
    args.d = &buffer;
    args.d_f = &buffer_bottom;
    args.type = DETECTION_DATA_FLIP_BOT;
	args.threads = 4;// 8;
    args.flip = joinNet.flip;
    ...
    pthread_t load_thread = load_data_flip(args);
    clock_t time;
    int count = 0;
    //while(i*imgs < N*120){
    while(get_current_batch(joinNet) < joinNet.max_batches){
        ...
        pthread_join(load_thread, 0);
        train = buffer;
        train_bottom = buffer_bottom;
        load_thread = load_data_flip(args);
...
#ifdef GPU
        if(ngpus == 1){
            loss = train_joint_network(net, bottomNet, joinNet, train, train_bottom);
        } else {
            loss = train_networks(nets, ngpus, train, 4);
        }
#else
		...
		// I Changed how often it saves the weights as it takes longer to train
		if (i % 5000 == 0) {
#ifdef GPU
        ...
        // Free the normal image data
        free_data(train);
        // Free the bottom image data
        free_data(train_bottom);
    }
#ifdef GPU
    if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
...
void run_detector(int argc, char **argv)
{
    ...
    char *testFile = find_char_arg(argc, argv, "-testFile", 0);
